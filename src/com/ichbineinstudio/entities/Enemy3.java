...
public class Enemy3 extends Entity{
	...
	private double life = 30, maxLife = 30;
	...
	public Enemy3(int x, int y, int width, int height, BufferedImage sprite) {
		...
		sprites[0] = Game.movements.getSprite(32*16, 1*16, 16, 16);
	    sprites[1] = Game.movements.getSprite(32*16 +16, 1*16, 16, 16);
	    sprites[2] = Game.movements.getSprite(32*16 +32, 1*16, 16, 16);
	    
	    sprites2 = new BufferedImage[3];
		sprites2[0] = Game.movements.getSprite(32*16, 5*16, 16, 16);
	    sprites2[1] = Game.movements.getSprite(32*16 +16, 5*16, 16, 16);
	    sprites2[2] = Game.movements.getSprite(32*16 +32, 5*16, 16, 16);
	    
	    sprites3 = new BufferedImage[3];
		sprites3[0] = Game.movements.getSprite(32*16, 9*16, 16, 16);
	    sprites3[1] = Game.movements.getSprite(32*16+16, 9*16, 16, 16);
	    sprites3[2] = Game.movements.getSprite(32*16+32, 9*16, 16, 16);
	    
	    sprites4 = new BufferedImage[3];
		sprites4[0] = Game.movements.getSprite(32*16, 13*16, 16, 16);
	    sprites4[1] = Game.movements.getSprite(32*16 + 16, 13*16, 16, 16);
	    sprites4[2] = Game.movements.getSprite(32*16 + 32, 13*16, 16, 16);
	}
		...
		//	if(Game.rand.nextInt(100) < 30) {
			...
			if((int)y < Game.player.getY()&& World.isFreeForEnemies(this.getX(),(int)(y+speed))
				...
				Game.player.life-=(Game.rand.nextInt(8)+1);
       ...
       /*     if(path == null || path.size() == 0) {
			...
			public boolean isColliding(int xnext,int ynext) { 
				...
				for(int i = 0; i < Game.enemies3.size(); i++) {
					...
					Rectangle targetEnemy3 = new Rectangle(e.getX()+ maskx,e.getY()+ masky, maskw,maskh);
				    if(enemy3Current.intersects(targetEnemy3)) {
				    	return true;
		...
		//	super.render(g);
			...
			g.fillOval(this.getX()- Camera.x + 2 , this.getY()- Camera.y + 14, 10, 3);
			}else if(Game.texture == "Texture_THREE") {
			g.setColor(new Color(40,40,40, 130));
		...
		//    g.fillRect(this.getX() + maskx - Camera.x, this.getY() + masky - Camera.y, maskw,maskh);
				...
				g.drawImage(Entity.ENEMY3_FEEDBACK_TEXTURE2,this.getX() - Camera.x,this.getY() - Camera.y,null);
			}else if(Game.texture == "Texture_THREE") {
				g.drawImage(Entity.ENEMY3_FEEDBACK_TEXTURE3,this.getX() - Camera.x,this.getY() - Camera.y,null);
			}else if(Game.texture == "Texture_FOUR") {
				g.drawImage(Entity.ENEMY3_FEEDBACK_TEXTURE4,this.getX() - Camera.x,this.getY() - Camera.y,null);
			}
