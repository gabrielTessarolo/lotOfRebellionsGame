...
public class Enemy2 extends Entity{
	...
	private double life = 15, maxLife = 15;
	...
	public Enemy2(int x, int y, int width, int height, BufferedImage sprite) {
		...
		sprites[0] = Game.spritesheet.getSprite(16, 4*16, 16, 16);
	    sprites[1] = Game.spritesheet.getSprite(0*16, 5*16, 16, 16);
	    sprites[2] = Game.spritesheet.getSprite(1*16, 5*16, 16, 16);
	    
	    sprites2 = new BufferedImage[3];
		sprites2[0] = Game.movements.getSprite(9*16, 4*16, 16, 16);
	    sprites2[1] = Game.movements.getSprite(10*16, 4*16, 16, 16);
	    sprites2[2] = Game.movements.getSprite(11*16, 4*16, 16, 16);
	    
	    sprites3 = new BufferedImage[3];
		sprites3[0] = Game.movements.getSprite(0, 15*16, 16, 16);
	    sprites3[1] = Game.movements.getSprite(1*16, 15*16, 16, 16);
	    sprites3[2] = Game.movements.getSprite(2*16, 15*16, 16, 16);

	    sprites4 = new BufferedImage[3];
		sprites4[0] = Game.movements.getSprite(9*16, 15*16, 16, 16);
	    sprites4[1] = Game.movements.getSprite(10*16, 15*16, 16, 16);
	    sprites4[2] = Game.movements.getSprite(11*16, 15*16, 16, 16);
	}
		...
		//	if(Game.rand.nextInt(100) < 30) {
			...
			public boolean isColliding(int xnext,int ynext) { 
				...
				for(int i = 0; i < Game.enemies2.size(); i++) {
				    ...
				    if(enemy2Current.intersects(targetEnemy2)) {
				    	return true;
		...
		public void render(Graphics g) {
			...
			g.fillOval(this.getX()- Camera.x + 2 , this.getY()- Camera.y + 13, 10, 5);
			}else if(Game.texture == "Texture_THREE") {
			g.setColor(new Color(40,40,40, 130));
			g.fillOval(this.getX()- Camera.x , this.getY()- Camera.y + 13, 15, 5);	
		...
		//    g.fillRect(this.getX() + maskx - Camera.x, this.getY() + masky - Camera.y, maskw,maskh);
				...
				g.drawImage(Entity.ENEMY2_FEEDBACK_TEXTURE3,this.getX() - Camera.x,this.getY() - Camera.y,null);	
				}else if(Game.texture == "Texture_FOUR") {
				g.drawImage(Entity.ENEMY2_FEEDBACK_TEXTURE4,this.getX() - Camera.x,this.getY() - Camera.y,null);	
				}
			}
